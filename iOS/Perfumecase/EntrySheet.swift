...
struct EntrySheet: View {
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Notes", text: $field2)
                        .accessibilityIdentifier("entryField2Field")
                    TextField("Fill %", text: $extraField)
    ...
    private func save() {
        if var e = editing {
            ...
            e.brand = field1
            e.notes = field2
            e.fillLevel = extraField
            e.notes = notes
            store.update(e)
        } else {
            let entry = BottleEntry(name: name, brand: field1, notes: field2, fillLevel: extraField, notes: notes)
            store.add(entry)
        }
        dismiss()
    }
